...
const Searchbar = ({value , onChange , onClear, itemsPerPage, OnItemsPerPageChange , currentPage , totalUsers} )=> {
    ...
    <div className="bg-gray-900 rounded-lg shadow-lg p-4 border border-gray-800 flex flex-col  md:flex-row md:items-center md:justify-between gap-4 ">
        <div className="relative flex-1">
            <Search size={18} className="absolute left-3 top-1/2 tranform -translate-y-1/2 text-gray-400 pointer-events-none" />
         ...
         {value && (
               <button onClick={onClear} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-white hover:border-green-500 outline-none">
                <X size={16} />
            </button>
         )}
        </div>
        ...
        {/* Rows per Pages & Info */}
         <div className="flex items-center gap-4 ">
            ...
            <span className="flex items-center gap-2 text-cyan-600">Showing {startUser} to {endUser} of {totalUsers} Users 
            </span>
            <div className="flex items-center gap-2 ">
                <label className="text-sm text-gray-400 ">Rows</label>
                <select className="px-3 py-1.5 bg-gray-800 border border-gray-700 text-white rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-green-500 outline-none text-sm" onChange={(e)=> OnItemsPerPageChange(Number(e.target.value))} >
